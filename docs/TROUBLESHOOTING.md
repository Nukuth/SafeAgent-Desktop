...
远程 UI 应通过 server token 读取：

```powershell
Invoke-RestMethod -Method Get -Uri "http://127.0.0.1:8080/api/devices/local-pc-1/heartbeat" -Headers @{ Authorization = "Bearer change-me" }
```
...
```text
1. 本地 worker 没有运行。
2. SAFEAGENT_DEVICE_ID 和查询的 device_id 不一致。
3. SAFEAGENT_WORKER_TOKEN 错误，worker 上报 heartbeat 被 auth.failed 拒绝。
4. 控制面服务不可达，worker 本地日志里会有 heartbeat_failed。
```

处理方式：

```text
1. 查看 E:\agents\logs\worker.jsonl。
2. 搜索 heartbeat_failed。
3. 如果 error.code = auth.failed，检查 worker token。
